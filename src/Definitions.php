...
final class Definitions {
     ...
     * user_id method may be "id" or "col1, col2".
     * @return string[]
     */
    public static function pkColumns(): array {
        $raw = trim('user_id method');
        if ($raw === '') {
            throw new \InvalidArgumentException('Definitions::pkColumns(): token user_id method must not be empty.');
        }
    ...
    public static function defaultOrder(): ?string {
        $c = trim('created_at DESC, user_id method DESC'); return $c !== '' ? $c : null;
    }
    ...
    public static function piiColumns(): array { return [ 'secret' ]; }
