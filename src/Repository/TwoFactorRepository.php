...
namespace BlackCat\Database\Packages\TwoFactor\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\TwoFactor\Definitions;
use BlackCat\Database\Packages\TwoFactor\Criteria;
use BlackCat\Database\Packages\TwoFactor\Dto\TwoFactorDto as Dto;
use BlackCat\Database\Packages\TwoFactor\Mapper\TwoFactorDtoMapper as RowMapper;
...
final class TwoFactorRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\TwoFactor\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'user_id', 'method' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\TwoFactor\Definitions::table());
            return $bulk->upsertMany($rows, [ 'user_id', 'method' ], [ 'secret', 'recovery_codes_enc', 'hotp_counter', 'enabled', 'last_used_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\TwoFactor\Dto\TwoFactorDto|null */
    public function getByUserIdAndMethod(int $userId, string $method, bool $asDto = false): array|\BlackCat\Database\Packages\TwoFactor\Dto\TwoFactorDto|null {
        $row = $this->getByUnique([ 'user_id' => $userId, 'method' => $method ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\TwoFactor\Mapper\TwoFactorDtoMapper::fromRow($row);
    }
    public function existsByUserIdAndMethod(int $userId, string $method): bool {
        $where = 't.' . Ident::q($this->db, 'user_id') . ' = :uniq_user_id' . ' AND ' . 't.' . Ident::q($this->db, 'method') . ' = :uniq_method';
        return $this->exists($where, [ 'uniq_user_id' => $userId, 'uniq_method' => $method ]);
    }
    /** @return int|string|null */
    public function getIdByUserIdAndMethod(int $userId, string $method) {
        $row = $this->getByUserIdAndMethod($userId, $method);
        return $row ? ($row['user_id method'] ?? null) : null;
    }
