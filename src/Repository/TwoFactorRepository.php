...
use BlackCat\Database\Packages\TwoFactor\Repository\TwoFactorRepositoryInterface;
...
class TwoFactorRepository implements TwoFactorRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'user_id', 'method' ];
    ...
    public function getByUserIdAndMethod(int $userId, string $method, bool $asDto = false): array|\BlackCat\Database\Packages\TwoFactor\Dto\TwoFactorDto|null {
        return $this->getByUnique([ 'user_id' => $userId, 'method' => $method ], $asDto);
    }
    ...
    public function getIdByUserIdAndMethod(int $userId, string $method) {
        $row = $this->getByUserIdAndMethod($userId, $method, false);
        if (!is_array($row)) { return null; }
        return $row['user_id method'] ?? null;
    }
